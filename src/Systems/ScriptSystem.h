...
void SetEntityRotation(Entity entity, double angle) {
  if (entity.HasComponent<TransformComponent>()) {
    ...
    Logger::Err("Trying to set the roation of an entity that has no transform component");
  }
}
...
void SetProjectileVelocity(Entity entity, double x, double y) {
  if (entity.HasComponent<ProjectileEmitterComponent>()) {
    ...
    Logger::Err("Trying to set the projectile velocity of an entity that has no projectile component");
  }
}
...
class ScriptSystem : public System {
 public:
  ...
  void CreateLuaBindings(sol::state& lua) {
    ...
    lua.new_usertype<Entity>("entity", "get_id", &Entity::GetId, "destroy", &Entity::Kill, "has_tag", &Entity::HasTag, "belongs_to_group", &Entity::BelongsToGroup);
