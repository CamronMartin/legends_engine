 ...
 public:
  ...
  void Update(SDL_Renderer* renderer, std::unique_ptr<AssetStore>& assetStore, const SDL_Rect& camera) {
    for (auto entity : GetSystemEntity()) {
      ...
      SDL_Surface* surface =
          TTF_RenderText_Blended(assetStore->GetFont("charriot-font-10"), healthText.c_str(), healthBarColor);
      ...
      SDL_Rect healthBarTextRectangle = {static_cast<int>(healthBarPosX), static_cast<int>(healthBarPosY) + 5,
